              ...
              const char *fmt, ...)
{
    ...
    if (fmt) {
         ...
         */
        if (cond)
            sprintf(buf, "Assertion failed: %s", cond);
        else
...
{
    return InterlockedExchangeAdd((volatile LONG *)ptr, increment);
}
...
{
    int x = InterlockedCompareExchange((volatile LONG *)addr, newvalue, oldvalue);
    return x != oldvalue;
}
