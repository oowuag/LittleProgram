...
const bdaddr_t BDADDR_ANY_const = {0, 0, 0, 0, 0, 0};
const bdaddr_t BDADDR_LOCAL_const = {0, 0, 0,  0xff, 0xff, 0xff};
...
static struct uuid_def uuid16_names[] = {
	...
	{ 0x1132, "Message Access (MAP) - MAS", mas_attrib_names, N_ELEMENTS(mas_attrib_names) },
...
{
	sdp_data_t *sdpdata = (sdp_data_t *)value;
	uint16_t attrId;
...
static int cmd_setattr(int argc, char **argv)
{
	...
	/* Do it */
	sess = sdp_connect(&BDADDR_ANY_const, &BDADDR_LOCAL_const, 0);
	if (!sess)
		return -1;
...
static void print_service_desc(void *value, void *user)
{
	...
	for (; p; p = p->next, i++) {
		...
		case SDP_UINT8:
			...
			{
				printf("    Channel: %d\n", p->val.uint8);
				struct search_context* cont = (struct search_context*)user;
				if (cont->found == 1) {
					cont->port = p->val.uint8;
					cont->found = 2;
				}
			}
			else
...
static void print_profile_desc(void *value, void *userData)
{
	...
	struct search_context* cont = (struct search_context*)userData;
	if(strcmp(cont->svc, UUID_str) == 0) {
		cont->found = 1;
	}
}
 ...
 */
static void print_service_attr(sdp_record_t *rec, struct search_context* context)
{
	...
	*/
	...
	if (sdp_get_profile_descs(rec, &list) == 0) {
		printf("Profile Descriptor List:\n");
		sdp_list_foreach(list, print_profile_desc, context);
		sdp_list_free(list, free);
	}
	if (sdp_get_access_protos(rec, &proto) == 0) {
		printf("Protocol Descriptor List:\n");
		sdp_list_foreach(proto, print_access_protos, context);
		sdp_list_foreach(proto, (sdp_list_func_t)sdp_list_free, 0);
		sdp_list_free(proto, 0);
	}
}
...
static int add_service(bdaddr_t *bdaddr, svc_info_t *si)
{
	...
	sess = sdp_connect(&interface, &BDADDR_LOCAL_const, SDP_RETRY_IF_BUSY);
	if (!sess)
		return -1;
...
static void inquiry(handler_t handler, void *arg)
{
	...
	for (i = 0; i < count; i++)
		handler(&ii[i].bdaddr, (search_context*)arg);
}
...
static int do_search(bdaddr_t *bdaddr, struct search_context *context)
{
	...
	for (; seq; seq = next) {
		...
		case DEFAULT_VIEW:
			...
			print_service_attr(rec, context);
			printf("\n");
			break;
		case TREE_VIEW:
			/* Display full tree */
...
int sdp_browse(const char* inAddr, const char* profileId)
{
	struct search_context context;
	/* Initialise context */
	memset(&context, '\0', sizeof(struct search_context));
	context.svc = const_cast<char*>(profileId);

	/* We want to browse the top-level/root */
	sdp_uuid16_create(&context.group, PUBLIC_BROWSE_GROUP);

	bdaddr_t bdaddr;
	estr2ba(const_cast<char*>(inAddr), &bdaddr);
	int ret = do_search(&bdaddr, &context);
	if (ret) {
		printf("do_search() error!\n");
	}

	int port = -1;
	if(profileId) {
		printf("## uuid:%s found:%d, port:%d\n", context.svc, context.found, context.port);
		port = context.port;
	}
