...
int card_add_param(VP *vp, int paramOff)
{
	...
	vp->params[vp->nParams * 2] = (CARD_Char *)(intptr_t)paramOff;
...
int card_set_paramvalue(VP *vp, int valueOff)
{
	...
	vp->params[(vp->nParams - 1) * 2 + 1] = (CARD_Char *)(intptr_t)valueOff;
...
int vcard_end_prop(VP *vp)
{
	...
	if (vp->cardProp)
	{
		...
		{
			vp->params[i * 2] = vp->lineBuf + (intptr_t)(vp->params[i * 2]);
			if (vp->params[i * 2 + 1])
				vp->params[i * 2 + 1] = vp->lineBuf + (intptr_t) vp->params[i * 2 + 1];

			/* check for encodings, needlessly complicated by solo value string
			   dumb spec choice */
			encoding = vp->params[i * 2];
			if (strcasecmp(encoding, "ENCODING") == 0)
